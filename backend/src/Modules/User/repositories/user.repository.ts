...
export class UserRepository {
    ...
    async updateUserItinerary(userId: string, itineraryId: string): Promise<IUser | null> {
        try {
            const user = await this.userModel.findById(userId);
            if (!user) {
            ...
            if (error instanceof Error) {
                console.error('Error in updateUserItinerary:', error.message);
                throw new Error(error.message);
            }
            throw new Error('An unknown error occurred in updateUserItinerary.');
        }
    }
}
